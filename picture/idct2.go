...
func idct2(src, s []int, stride int) {
   ...
   for i := 0; i < 8; i++ {
      if s[8] | s[16] | s[24] | s[32] | s[40] | s[48] | s[56] == 0 {
         ...
         n.x2 += 512
         n.x3 += 512
         v[8*0] = (n.x0 + n.t3[0]) >> 10
         v[8*1] = (n.x1 + n.t2[0]) >> 10
         v[8*2] = (n.x2 + n.t1[0]) >> 10
         v[8*3] = (n.x3 + n.t0[0]) >> 10
         v[8*4] = (n.x3 - n.t0[0]) >> 10
         v[8*5] = (n.x2 - n.t1[0]) >> 10
         v[8*6] = (n.x1 - n.t2[0]) >> 10
         v[8*7] = (n.x0 - n.t3[0]) >> 10
      }
      s = s[1:]
      v = v[1:]
   }
